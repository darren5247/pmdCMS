...
export default {
    async beforeUpdate(event) {
        // Store original key_signature title before update to use in afterUpdate
        const originalKeySignature = await strapi.entityService.findOne('api::key-signature.key-signature', event.params.where.id);
        event.state.wasPublished = originalKeySignature.publishedAt;
    },
    ...
    async afterUpdate(event) {
        ...
        try {
            ...
            const fieldsToBeUpdated = await Promise.all(workIdsToBeUpdated.map(async workId => {
                ...
                const keySignatures = work.keySignatures?.filter(c => c.publishedAt !== null);

                return {
                    id: workId.toString(),
                    key_signatures: keySignatures.map(c => c.title) || [],
                };
            })) || [];
            ...
            strapi.log.error('Failed to sync key_signature works to Typesense:', error);
        }
    },

    async  beforeDelete(event) {
        const itemToBeDeleted = await strapi.entityService.findOne('api::key-signature.key-signature', event.params.where.id, {
            populate: {
                works: {
                    fields: ['id']
                }
    ...
    async beforeDeleteMany(event) {
        const keySignatureIds = event.params.where.$and[0].id.$in;

        const itemsToBeDeleted = await strapi.entityService.findMany('api::key-signature.key-signature', {
            filters: {
                id: {
                    $in: keySignatureIds
                }
