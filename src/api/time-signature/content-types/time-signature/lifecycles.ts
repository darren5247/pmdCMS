...
export default {
    async beforeUpdate(event) {
        // Store original time_signature title before update to use in afterUpdate
        const originalTimeSignature = await strapi.entityService.findOne('api::time-signature.time-signature', event.params.where.id);
        event.state.wasPublished = originalTimeSignature.publishedAt;
    },
    ...
    async afterUpdate(event) {
        ...
        try {
            ...
            const fieldsToBeUpdated = await Promise.all(workIdsToBeUpdated.map(async workId => {
                ...
                const timeSignatures = work.timeSignatures?.filter(c => c.publishedAt !== null);

                return {
                    id: workId.toString(),
                    time_signatures: timeSignatures?.map(c => c.title) || [],
                };
            })) || [];
            ...
            strapi.log.error('Failed to sync time_signature works to Typesense:', error);
        }
    },

    async  beforeDelete(event) {
        const itemToBeDeleted = await strapi.entityService.findOne('api::time-signature.time-signature', event.params.where.id, {
            populate: {
                works: {
                    fields: ['id']
                }
    ...
    async beforeDeleteMany(event) {
        const timeSignatureIds = event.params.where.$and[0].id.$in;

        const itemsToBeDeleted = await strapi.entityService.findMany('api::time-signature.time-signature', {
            filters: {
                id: {
                    $in: timeSignatureIds
                }
