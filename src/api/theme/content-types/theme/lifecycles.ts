...
export default {
    async beforeUpdate(event) {
        // Store original theme title before update to use in afterUpdate
        const originalTheme = await strapi.entityService.findOne('api::theme.theme', event.params.where.id);
        event.state.wasPublished = originalTheme.publishedAt;
    },
    ...
    async afterUpdate(event) {
        ...
        try {
            ...
            const fieldsToBeUpdated = await Promise.all(workIdsToBeUpdated.map(async workId => {
                ...
                const themes = work.themes?.filter(c => c.publishedAt !== null);

                return {
                    id: workId.toString(),
                    themes: themes?.map(c => c.title) || [],
                };
            })) || [];
            ...
            strapi.log.error('Failed to sync theme works to Typesense:', error);
        }
    },

    async  beforeDelete(event) {
        const itemToBeDeleted = await strapi.entityService.findOne('api::theme.theme', event.params.where.id, {
            populate: {
                works: {
                    fields: ['id']
                }
    ...
    async beforeDeleteMany(event) {
        const themeIds = event.params.where.$and[0].id.$in;

        const itemsToBeDeleted = await strapi.entityService.findMany('api::theme.theme', {
            filters: {
                id: {
                    $in: themeIds
                }
